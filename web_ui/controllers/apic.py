...
class ApicController:
    ...
    def getInventory(self, pod_dn):
        ...
        switches = self.makeCall(
            p_url='/api/node/mo/' + pod_dn + '.json?query-target=children',
    ...
    def getUnmanagedNodes(self):
        neighbors = self.makeCall(p_url='/api/node/class/fabricLooseNode.json', method="GET").json()['imdata']
        return neighbors

    def getInterfaceLldpNeighbor(self, if_id):
        neighbors = self.makeCall(
            p_url="/api/node/mo/topology/pod-1/node-203/sys/lldp/inst/if-[" + if_id + "].json?query-target=children&target-subtree-class=lldpAdjEp",
            method="GET").json()['imdata']
        return neighbors

    def getActiveInterfaces(self, switch_dn):
        interfaces = self.makeCall(
            p_url='/api/node/class/' + switch_dn + '/l1PhysIf.json?rsp-subtree=children&rsp-subtree-class=ethpmPhysIf&query-target=children&query-target-filter=and(eq(ethpmPhysIf.operSt,"up"))&order-by=l1PhysIf.id',
            method="GET").json()['imdata']
        return interfaces

    def getLldpNeighbors(self,switch_dn):
        neighbors = self.makeCall(
            p_url='/api/node/class/' + switch_dn + '/lldpIf.json?rsp-subtree=children&rsp-subtree-class=lldpIf,lldpAdjEp&rsp-subtree-include=required&order-by=lldpIf.name',
            method="GET").json()['imdata']
        return neighbors